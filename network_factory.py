...
def from_filtered_import(import_res: Result, filtered_direct_succession: Dict[str, Dict[str, float]], filtered_out_two_loop: Dict[str, Dict[str, float]],
    ...
    for src, targets in filtered_direct_succession.items():
        ...
        def check_node_insert_parameters(src: str, filtered_out_two_loop: Dict[str, Dict[str, float]],
                         parallel_tuples: List[Tuple[str, str]], self_loop_events: List[str]):
            ...
            return self_looped, and_paralleled_with, is_in_two_loop_main, in_two_loop_feedback_with

        node_insert_parameters = check_node_insert_parameters(src, filtered_out_two_loop, parallel_tuples, self_loop_events)
        ...
        for target in targets:
            node_insert_parameters = check_node_insert_parameters(target, filtered_out_two_loop, parallel_tuples,
                                                                  self_loop_events)
